...
def invoke(
    ...
    print(f"invoke has functionInput {functionInput}")
    ...
    # google
        ...
        try:
            ...
            print(f"invoking google cloud function failed {str(e)}")

    # tinyFaaS
    elif provider.lower() == "tinyfaas":
        print("next provider tinyfaas")
    ...
    # error
        ...
        print("no next provider")
        raise Exception("unknown provider, cannot invoke")

    return None


def invokeAWS(workflowStep: Dict[str, Any], workflow: Optional[Dict[str, Any]], functionInput: Optional[Dict[str, Any]], objectName: Optional[str]) -> None:
    """
    ...
    if workflowStep:
        ...
        result = requests.post(
            ...
            json={
                "workflow": workflow,  # TODO i changed stuff here
                "body": ("" if functionInput is None else functionInput),
...
def invokeGCP(projectId: str, credentials: Dict[str, Any], topic: str, args: Dict[str, Any]) -> str:
    print(f"trying to invoke google function for inputs projectId {projectId}, credentials {credentials}, topic {topic}, args {args}")
    # FIXME for now: import it here so the imports don't break the lambda functions
