...
def handler(dataPath: Optional[str], functionInput: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    print(f"end to end latency {endToEndLatency}")
...
# TODO last change: in the data collector, adapt the format string for the insert query to expect int (%d) instead of float!!
#  also check that that is what the table wants but I think it does

# - experiment laufen lassen
# - hoffen dass baseline auch klappt
# - plot machen wegen morgen paper sprint
# - für 2. use case was ausdenken und plan machen
